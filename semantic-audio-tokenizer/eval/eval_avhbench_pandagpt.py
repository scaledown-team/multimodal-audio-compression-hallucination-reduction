...
from .core.registry import MODEL_REGISTRY, set_model_registry
from .core.datatypes import QARecord, CapRecord
from .core.pandagpt_loader import load_pandagpt_baseline
from .core.io_utils import group_qa_by_type, load_json_any
from .core.inference import multimodal_inference_baseline
from .core.metrics import compute_classification_metrics, compute_caption_overlap_metrics, normalize_yesno
from .core.persistence import save_eval_outputs
...
def evaluate_captions(caps: List[dict]) -> tuple[list[list[str]], list[str], list[str], list[CapRecord]]:
    ...
    for i, item in enumerate(tqdm(caps, desc="Caption generation (baseline)")):
        ...
        if not refs: 
            continue
        out = multimodal_inference_baseline(audio_path, video_path, "Describe what you see and hear.")
...
def main():
    ...
    baseline_model, video_pre, audio_pre = load_pandagpt_baseline(device=MODEL_REGISTRY["device"])
    set_model_registry(
    ...
    logger.info("PandaGPT baseline wired.")
